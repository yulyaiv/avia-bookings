...
select date_trunc('month', book_date), round(sum(total_amount)* 100./ sum(sum(total_amount)) over (), 2) as "percentage_difference" 
from bookings 
group by 1 
order by 1 
...
with cte as (
	select f.flight_id, f.departure_airport as "airport", f.actual_departure as "flight_date",
	    count(s.seat_no) - count(bp.boarding_no) as "empty_seats"
	from flights f
	...
	having count(bp.boarding_no) = 0 and count(f.flight_id) > 1)
select airport, flight_date, empty_seats, sum(empty_seats) over (partition by airport order by flight_date)
from cte
